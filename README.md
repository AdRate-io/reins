...
**0.3.0** — twelve packages, ~50k lines of TypeScript (tests included), 1,237 tests, every default measured on real models (see `examples/eval`). New in 0.3: tools contributed by sockets — MCP servers in particular — can sit on the `lazyTools()` menu and load on demand (`SocketSetup.tools`; see the tools-mcp README for the TikTok recipe). New since 0.1: `@reinsjs/lowering-fetch` (a zero-dependency lowering layer for OpenAI Chat Completions, Anthropic Messages and OpenAI Responses — now the recommended one, and what every example uses), lazy tool discovery with Anthropic's native deferred loading, approvals that expire, a run registry that spans processes, an `onEvent` observer on the HTTP handler, MCP auth for expiring credentials, and English runtime messages throughout. Public types are frozen within a minor line; breaking changes bump the minor version until 1.0.
