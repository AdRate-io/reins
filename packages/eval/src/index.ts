/**
 * @reinsjs/eval —— eval harness（技术方案 §13，M2 E1）。
 *
 ...
 * - arms：内置 none / threshold 两臂，以及缩窗口的 Lowering 包装；模型自决臂由调用方用 @reinsjs/brain 组
 * - runner：fixture × 臂 × 重复的对照运行器，含审批代答、预算续跑、handoff 跟随、分叉探针问答
 * - gate：PRD §7 门槛 2 / P8 的四条硬规则
 * - report：Markdown 表
 *
 * 只依赖 @reinsjs/core，零 node:*；文件读写与 CLI 在调用方（examples/eval）。
 */
export * from "./arms.js"
export * from "./gate.js"
