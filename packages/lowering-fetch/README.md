...
- **Thinking** is replayed only when the event carries a `signature` from the same provider, API **and model** (`thinking` / `redacted_thinking` blocks); unsigned thinking (an interrupted stream), thinking from another provider, or thinking from another model of the same provider is `dropped` and declared — it is not turned into visible text (a signature is only valid for the model that produced it: replaying one across models is a 400). Empty-text thinking blocks with a signature (`display: "omitted"`) are kept and replayed.
