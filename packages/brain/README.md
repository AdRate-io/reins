...
- **Loading a tool re-warms the prompt cache once.** With `lazyTools()` the request's tool list changes on the turn after a `tool_find`, and Anthropic invalidates the tools / system / messages cache prefix whenever the tool list changes — so that one request is a full cache write, after which hits resume. The rules text asks the model to load everything a task needs in one call; measured numbers are in `spikes/d1-lazy-tools-cache/`. `tool_find` results are trusted like the system prompt (`trust: "system"`): tool descriptions and schemas are host configuration.
