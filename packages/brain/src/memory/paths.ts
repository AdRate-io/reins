/**
 * 记忆路径的限定与规范化（技术方案 §9.6 / §13：记忆路径严格限定，拒绝穿越）。
 *
 * 模型给的路径先过这里再碰存储。规则宁严勿宽：
 * - 必须是 `/memories` 本身或以 `/memories/` 开头（大小写敏感）
 * - 不接受 `.` / `..` 段、反斜杠、百分号编码（`%2e%2e%2f` 一类）、控制字符
 * - 重复斜杠与末尾斜杠折叠掉，得到唯一的规范形态 —— 同一个文件只有一个键
 * - 段首尾不能有空白（"a " 与 "a" 在存储里是两个键，模型分不出来）
 *
 * 存储层（MemoryStore）只见规范路径（可能再加宿主的命名空间前缀），所以后端不需要再做一遍防穿越；
...
export function resolveMemoryPath(raw: unknown, field = "path"): string {
  const reject = (why: string): never => {
    throw new RangeError(
      `Invalid \`${field}\`: ${why}. Paths must be absolute and stay under ${MEMORY_ROOT}.`,
    )
  }
  if (typeof raw !== "string") return reject("expected a string")
  const p = raw.trim()
  if (p.length === 0) return reject("it is empty")
  if (p.length > MAX_MEMORY_PATH_LENGTH)
    return reject(`it is longer than ${MAX_MEMORY_PATH_LENGTH} characters`)
  ...
  if (p !== MEMORY_ROOT && !p.startsWith(`${MEMORY_ROOT}/`)) {
    return reject(`it must be ${MEMORY_ROOT} or start with ${MEMORY_ROOT}/`)
  }
