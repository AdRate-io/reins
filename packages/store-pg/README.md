...
`pgStores` returns `{ log, blobs, memory }`, ready for `createAgent({ store })`. PGlite (`new PGlite("./data")`) is a drop-in for local development and tests — `examples/team` uses it.
