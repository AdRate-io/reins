...
export class PgEventLog implements EventLog {
  ...
  async append(events: readonly Event[]): Promise<void> {
    ...
    for (const e of events) {
      ...
      if (e.seq !== expected) {
        throw new StoreError(
          "seq_conflict",
          `session ${sessionId}: seq is not contiguous within the batch, expected ${expected}, got ${e.seq}`,
          {
            sessionId,
            expected,
            got: e.seq,
    ...
    if (first.seq < 1) {
      throw new StoreError("seq_conflict", `session ${sessionId}: seq must start at 1, got ${first.seq}`, {
        sessionId,
        got: first.seq,
      })
    }
    ...
    if (inserted === 0) {
      ...
      throw new StoreError(
        "seq_conflict",
        `session ${sessionId} expected seq=${last + 1}, got ${first.seq}`,
        {
          sessionId,
          expected: last + 1,
          got: first.seq,
