...
12 个包、约 5 万行 TypeScript（源码 2.9 万 + 测试 2.1 万）、1231 个用例。**我的使命：守护这套我们共同创造的系统，让它在每一次模型换代后都更对，而不是更旧。**
...
- **量缓存的探针必须给系统提示加一次性盐值** — Anthropic 缓存 5 分钟 TTL 且命中续期，同一脚本 10 分钟内重跑，对照臂"应归零"的请求会读到上一遍写的前缀（L1 P3 假阴性一次）；"cache_read > 0" 的正向断言同样可能是上一遍的功劳。**盐必须落在 `cache_control` 断点之内**（P9 首测追加成 system 新块、落在断点之后，等于没加）；要隔离的是跨运行，同组各臂应共用同一个盐——臂间能否互相命中本身就是判据。
- **`defer_loading` 的定义取回之前不进缓存前缀、不计费，取回后在历史段展开、按普通 input 计费**（L1 P9 六臂 6/6，同组共用一次性 run 盐：带标臂写 8499，只留 tool_find 的删件臂**整段命中 8499**——两者计费前缀逐字相同；换掉三件的 description 仍整段命中，连定义内容都不进前缀；去标实发写 8675，差 **176** = 三件定义；反序复跑同样成立）— 请求体仍全量下发（服务端留作展开与校验），但排除出计费前缀；官方原文 "excludes deferred tools from the system-prompt prefix"。CF 网关透传路径对 `count_tokens` 端点不转凭证（401 要 `x-api-key`），量化 token 只能打真模型对照臂。
- **Bun / Deno / Vercel Edge 四环境矩阵（`spikes/runtime-matrix`，2026-09-15）：库代码一行不改，三处差异全在宿主配置** — pi-ai 的 UA 用 `process.getBuiltinModule("node:os")` 读 `os.release()`，Deno 见 `process.versions.node` 就走这条路、要 `--allow-sys=osRelease`（fetch 版只要 `--allow-net`）；`openai` SDK 6.40 的 `EdgeRuntime` 分支裸读 `globalThis.process.version`（Anthropic SDK 同处有 `?.`），Edge 运行时必须有 `process` 全局（Vercel 线上有 `process.env`，裸 `edge-runtime` 模拟器没有）；`Bun.serve` 缺省 `idleTimeout` 10 秒会无声掐断慢模型调用。真 Vercel Edge 生产部署（Boss Hobby 账号，项目 `reins-runtime-probe`）7/7：线上自带 `process`（无 `versions.node`）与 `Buffer`、`node:crypto` 可进、`node:fs` 不可，比裸模拟器宽；Vercel 2026 起不再推荐 Edge 运行时（缺省 Node 24 Fluid Compute），`runtime:"edge"` 仍支持。npm 装的 `bun` / `deno` bin 是 shell 包装，kill 要按进程组。
