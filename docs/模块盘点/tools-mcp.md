...
| `packages/tools-mcp/src/connection.ts` | `McpConnection`：`ensure()` 懒建 / 并发合流 / `onclose` 清引用 / 下次需要时重建；`listTools`（`cacheMode: "bypass"`，绕过 SDK 列表缓存）；`callTool`（timeout + signal）；`close` |
