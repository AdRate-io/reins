...
| `packages/store-sqlite/src/schema.ts` | `SQLITE_SCHEMA_SQL` 三张建表语句 + 幂等 `migrateSqlite(db)` |
...
| `packages/store-sqlite/src/memory-store.ts` | `SqliteMemoryStore`：path→content KV，前缀查询用 `substr` |
| `packages/store-sqlite/src/stores.ts` | `sqliteStores(db, opts)` 组装 `Stores`，缺省建表 |
...
| `packages/store-pg/src/schema.ts` | `PG_SCHEMA_STATEMENTS` 逐条 DDL + `PG_SCHEMA_SQL` 整段 + 幂等 `migratePg(client)` |
...
| `packages/store-pg/src/memory-store.ts` | `PgMemoryStore`：`left()` 前缀匹配，`ORDER BY path COLLATE "C"` |
| `packages/store-pg/src/stores.ts` | `pgStores(client, opts)` 组装 `Stores`（异步，因为建表要 await） |
| `sqlite.test.ts` / `pg.test.ts` | 各自挂三份一致性套件，再加特有行为：持久化与 WAL、`migrate:false`、与内存实现 deepEqual、千级事件分页、并发写 `seq_conflict`、前缀含 `%` `_` 的字面匹配；pg 缺省跑 PGlite，设 `REINS_PG_URL` 再对真库跑一遍 |
...
**装配与选项。** `sqliteStores(db, opts)` 同步返回 `Stores`，`pgStores(client, opts)` 返回 `Promise<Stores>`（建表要 await）。两者的 opts 一样只有两项：`migrate`（缺省 `true`，起步幂等建表；自己管迁移的宿主传 `false`）和 `now`（缺省 `() => Date.now()`，注入给 BlobStore 与 MemoryStore 的时间戳；EventLog 不用它，事件的 `at` 由调用方带来）。`openSqlite(path, opts)` 另有 `wal`（文件库缺省开）与 `busyTimeoutMs`（缺省 5000），`:memory:` 不适用这两项，`PRAGMA foreign_keys = ON` 无条件开。

**表名目前写死** 为 `reins_events` / `reins_blobs` / `reins_memory`，DDL 与全部查询都是字面量，没有前缀或 schema 选项。这是 TASKS 的 **P2「memory 隔离收口」** 待办：要把表名做成 `sqliteStores` / `pgStores` 的可配置项（缺省不变，建表语句同步）。在那之前，同一个库里跑多套 reins 会共用同三张表。
