...
5. 预校验分支：只有 `body.resume !== undefined`、`decisions` 非空、或 `input` 是草稿三者之一成立时才进。里面依次 `readTimeline(log, sessionId, { registry })` → （有 resume 时）`validateResume`，其 `configHash` 用 `computeConfigHash({ model, ...resolveSocketContributions(agent) })` 算，与 `runLoop` 起步同一份算法 → 每条 `decisions` 必须指向 `pendingToolCalls(timeline)` 里的调用，否则抛 `RunStateError("unknown_tool_call")` → `checkInput(body.input, pending, contributions.tools)`。`RunStateError` 一律翻成 409 + 其 `code`。
