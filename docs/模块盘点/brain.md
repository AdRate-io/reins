...
| lazy-tools | beforeModel、beforeTool | 工具 `tool_find`（`resultTrust: "system"`）与规则提示 `LAZY_TOOL_RULES` + 菜单（每件 `lazy: true` 的工具一行（宿主 + 注册在它之前的 Socket 贡献，2026-09-22 起）），按 `SocketSetup` 算一次的函数形态 | opt-in：装了即开（无 eval 不进 README 推荐，eval 见 `examples/eval/fixtures/tool-discovery`） | 宿主没有一件 `lazy: true` 的工具、或已有同名 `tool_find` → **工具与菜单都不注册**并告警一次 |
...
| `packages/brain/src/lazy-tools/lazy-tools.test.ts` | lazy-tools 的全部用例（摘要 / 菜单 / 入参 / 重建 / 上界纯函数；静态贡献与告警；与 runLoop 集成：首轮隐藏、取回后可见、block 文案、跨 run 重建、被拦的取回不算、前置 Socket（MCP 形态）的 lazy 工具进菜单并被藏、后置 Socket 的不藏且告警顺序、前置 Socket 同名 `tool_find` 不注册、× spill；原生路径：全表 + deferLoading、取回后仍延迟、未取回直调仍 block、取回那轮被折出视图则不延迟） |
...
1. run 起步：`tools` 与 `systemPrompt` 共用按 `SocketSetup` 缓存的解析——到本模块为止已并入的工具表（`setup.tools`：宿主 + 注册在前面的 Socket 贡献，2026-09-22 起）里已有 `tool_find` 或没有一件 `lazy: true` 的工具就都返回 undefined（不注册）并告警一次；否则 `lazyMenuOf(setup.tools)` 挑出 lazy 工具按 name 排序，把这些 Tool 对象记进 `WeakSet`（"菜单工具"），并按这次菜单造一个 `tool_find`（名字 / 说明 / schema 逐字固定，闭包里只有菜单不同；`resultPolicy.maxTokens = toolFindResultBound`）。
2. 系统提示片段 = `LAZY_TOOL_RULES`（先取回再动手、一次取全、只取任务要的、取过的不重取、别按摘要猜参数）+ `Available on request:` 每项一行 `- name: 摘要`；进 configHash，run 内不变。
3. `beforeModel`：菜单名 = `ctx.tools` 里 `lazy === true` 且在 WeakSet 里的（标了 lazy 却不在 WeakSet 里的是注册在本模块之后的 Socket 贡献的：不藏、告警一次点名并指出顺序）；`revealedLazyTools(ctx.timeline, 菜单名)` 把所有 `tool_find` 的 tool_call 与其 `isError === false` 的 tool_result 按 toolCallId 配对，取 `args.names ∩ 菜单名`；被藏的（在菜单里且未取回）记进 `WeakMap<TurnContext, Map<name, Tool>>`。然后按 `ctx.capabilities.deferredTools` 分两路（L1）：**原生路径**——工具表不动（全表下发），返回 `{ deferredTools }`：没取回的延迟，取回了且取回那轮还在 `ctx.events`（本轮视图）里的也延迟（厂商从历史里的 `tool_reference` 展开），取回了但那轮已被折出视图的不延迟（定义进 tools 块）；**过滤路径**——本轮工具表 = 去掉被藏的，返回 `{ tools }`。
...
**菜单收"到本模块为止已并入的工具"，注册在它之后的 Socket 贡献的 lazy 工具不藏**（D1 → 2026-09-22 改）— D1 时静态贡献阶段各 Socket 互不可见（`SocketSetup` 只有 `hostTools`），菜单只能列宿主工具。AdRate 接 TikTok MCP flat 模式（按 `readOnlyHint` 剔写工具后仍约 200 件只读）是等到的第二个样本，core 给 `SocketSetup` 加了 `tools`（到本 Socket 为止已并入的表），菜单改从它挑——`lazyTools()` 注册在 `mcpTools()` 之后即可，tools-mcp 不加接口（`override` 一行标 lazy）。仍用 `WeakSet<Tool>` 按对象同一性记菜单工具而不是按名字或 `lazy` 字段：注册在后面的 Socket 贡献的 lazy 工具不在其中，不藏（菜单上没有等于消失），`beforeModel` 告警一次点名指出顺序。MCP 工具的说明经 `tool_find` 取回仍 trust=system——它本就直接进 tools 块、同一信任层级。配方与反面用例在 `packages/tools-mcp/src/readonly-lazy.recipe.test.ts`。
