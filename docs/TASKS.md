...
- [ ] MCP 工具进 lazy-tools 菜单（`SocketSetup` 加"此前已并入的工具"）——TikTok layer 模式已在服务端做了渐进披露，44 件全表下发扛得住，等真出现"MCP 工具多到必须懒发现"的第二个样本再议
