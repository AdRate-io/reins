...
- [ ] **MCP 工具进 lazy-tools 菜单（0.3 第一个任务，2026-09-22 立项）**：真需求来自 AdRate 接 TikTok MCP 走 flat 模式——按 `readOnlyHint` 剔写工具后仍有约 200 件只读，必须菜单化。改法：core `SocketSetup.tools`（到本 Socket 为止已并入的工具表）+ lazy-tools 从它挑 `lazy: true` + 顺序反了告警；tools-mcp 不加接口，`override` 一行 + README 配方（含 code≠0 转 isError）；core / brain / tools-mcp 各一条 changeset。设计见 DECISIONS 2026-09-22
