  ...
  for (const m of payload.messages) {
    const text =
      typeof m.content === "string"
        ? m.content
        : Array.isArray(m.content) && m.content.length === 1 && m.content[0].type === "text"
          ? m.content[0].text
          : null
...
console.log(
  `→ system 位于索引 ${bSys}；满足官方摆放规则（前一条 user，后一条 assistant 或结尾）？${okPlacement}`,
)
console.log(
  "   注意：本例中 system 后面紧跟 user，官方规则判为 400。真实降级层需要把 system_note 与下一条 user 的顺序调整为 user→system 或合并。",
)
