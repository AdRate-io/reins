...
| `f2-anthropic-live/` | F2 | 仓库根 `pnpm build`，然后 `node spikes/f2-anthropic-live/probe.mjs [haiku\|opus\|all]`；配置自动从信息文件读 | `@reinsjs/lowering-fetch` 的 Anthropic Messages 线在 core `runLoop` 上经 CF 网关打官方模型：两次工具调用、Haiku 4.5 显式 thinking 的签名块第二个请求起原样回放被接受、Opus 5 上宿主中途注入的 system_note 以 `role:"system"` 到达（暗号进最终答案）且请求体不带 thinking 参数、系统提示凑过 4096 token 后第二个请求起 cacheRead > 0（我们自己打的三处断点在官方上真命中）、有损落点只有声明过的几种。Haiku / Opus 各 9/9（2026-09-15） |
| `f3-responses-live/` | F3 | 仓库根 `pnpm build`，然后 `node spikes/f3-responses-live/probe.mjs [mini\|full\|all]`；配置自动从信息文件读 | `@reinsjs/lowering-fetch` 的 OpenAI Responses 线在 core `runLoop` 上经 CF 网关打官方模型：两次工具调用、每轮产出带 `encrypted_content` 的 reasoning 项且第二个请求起整项原样回放被接受、宿主中途注入的 system_note 以 `role:"developer"` 到达（暗号进最终答案）、每个请求 `store:false` 且 function_call 带 `fc_` 项 id 回放、第二个请求起 cacheRead > 0、有损落点只有声明过的几种。gpt-5-mini 9/9、gpt-5.4 9/9（2026-09-15；gpt-5.4 第一遍自动缓存未命中 8/9、重跑命中——OpenAI 自动缓存尽力而为，判据留着但偶发 miss 不算我们的错） |
| `mcp-removed-tool-history/` | P1 ⑧ | 仓库根 `pnpm build`，然后 `node spikes/mcp-removed-tool-history/probe.mjs`；密钥自动从信息文件读 | 历史含已移除工具的 tool_use / tool_result 时：DeepSeek（Anthropic 协议直连）与 aireiter gpt-5.5（OpenAI Responses）在"工具表里有别的工具"与"工具表为空"两个变体下都接受且模型正确作答、只列当前工具；aireiter 的 Claude 端点两变体都回 "stream ended without a stop reason"（网关改写截断，非协议拒绝）；**官方 Anthropic 经 CF 网关 2026-09-15 补测（`l1-deferred-tools/` P8a/b）：两变体都接受**。结论：平台不需要"删工具新会话生效"的不对称规则；判据是模型产出而非状态码 |
| `l1-deferred-tools/` | L1 | `node spikes/l1-deferred-tools/probe.mjs <haiku\|opus\|deepseek\|all>`（裸 fetch 测厂商规矩）；`pnpm build` 后 `node spikes/l1-deferred-tools/live.mjs [haiku\|opus]`（runLoop + lazyTools + lowering-fetch 端到端）；配置自动从信息文件读 | Anthropic 官方 `defer_loading` + 自定义工具结果里的 `tool_reference`（GA 无 beta 头）：延迟工具对模型不可见（菜单必要）、取回后第 2 / 3 个请求 cache_read > 0（Haiku 8497 / Opus 4062）而老路子归零；厂商规矩——tool_result 内引用不能与文本混放、同条 user 里 tool_result 必须排最前、引用指向 tools 里没有的名字 400（整段历史都校验）、defer_loading 工具不能带 cache_control、全表延迟 400。顺带补测官方对"历史含已移除工具"接受。DeepSeek 兼容端口忽略 defer_loading。端到端 Haiku / Opus 各 9/9，同会话第二次 run 直接调用取回过的工具。详见目录内 README |
...
| `runtime-matrix/` | 0.1 之后「四环境验证」 | 仓库根 `pnpm build`；`cd spikes/runtime-matrix && pnpm i --ignore-workspace`（装 Bun / Deno / edge-runtime / esbuild 到本目录，不动全局）；然后 `NO_PROXY=127.0.0.1,localhost node spikes/runtime-matrix/run.mjs [--live] [--only node,bun,deno-min,deno-sys,edge,edge-process]` | **Bun 1.4 / Deno 2.9 / Vercel Edge（edge-runtime 4.0 裸 vm）托管 edge-runtime-check 的同一个 fetch 探针打 dist，每格内容核对**：fetch 版三条线真模型四个运行时全通；pi 版全通的条件——Deno 要 `--allow-sys=osRelease`（pi-ai 的 UA 读 `os.release()`，fetch 版只要 `--allow-net`）、Edge 上 `openai` SDK 裸读 `process.version`（Vercel 承诺的 `process.env` 垫片即够）；Bun 宿主要把 `Bun.serve.idleTimeout` 拉过缺省 10 秒。Node 22 对照臂 10/10 作基准。**真 Vercel Edge 生产部署 7/7**（`build-vercel.mjs` / `deploy-vercel.mjs` / `probe-vercel.mjs`，Boss 的 Hobby 账号；线上有 `process` / `Buffer`，openai SDK 路径直接通）。详见目录内 README |
